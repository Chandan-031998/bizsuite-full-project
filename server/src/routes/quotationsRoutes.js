...
/**
 * CREATE quotation  (Admin + Accounts)
 */
router.post("/", authorizeRoles("admin", "accounts"), async (req, res) => {
  try {
    const { client_id, quote_number, quote_date, total_amount, status, notes } =
      req.body;
...
/**
 * LIST quotations  (Admin + Accounts + Sales VIEW)
 */
router.get("/", authorizeRoles("admin", "accounts", "sales"), async (_req, res) => {
  try {
...
/**
 * GET single quotation  (Admin + Accounts + Sales VIEW)
 */
router.get(
  "/:id",
...
/**
 * UPDATE quotation  (Admin + Accounts)
 */
router.put("/:id", authorizeRoles("admin", "accounts"), async (req, res) => {
  try {
    const { client_id, quote_number, quote_date, total_amount, status, notes } =
      req.body;

    const existing = await get("SELECT id FROM quotations WHERE id = ?", [
      req.params.id,
    ]);
...
/**
 * DELETE quotation  (Admin + Accounts)
 */
router.delete("/:id", authorizeRoles("admin", "accounts"), async (req, res) => {
  try {
...
/**
 * CONVERT quotation → invoice  (Admin + Accounts)
 */
router.post(
